...
def compute_rainbow_color(group_id: int = 0, freq: float = 0.4):
    color = np.zeros(3)
    color[0] = math.sin(freq * group_id + 0) * 0.5 + 0.5
    color[1] = math.sin(freq * group_id + 2) * 0.5 + 0.5
    color[2] = math.sin(freq * group_id + 4) * 0.5 + 0.5
    color *= 255.0
...
class GaussianSplattingVisualizer:
    def __init__(
        ...
        self.cameras_to_reserve = cameras_to_reserve
    ...
    def setup_ui(self):
        ...
        # gui_theme_code = self.server.gui.add_markdown("no theme applied yet")

        # GUI elements for controllable values.
        # titlebar = self.server.gui.add_checkbox("Titlebar", initial_value=True)
        ...
        with self.server.gui.add_folder("📷 Camera Controls"):
            ...
            self.gui_frames = self.server.gui.add_slider(
                "Max Frames",
                min=0,
                max=len(self.images)+self.cameras_to_reserve,
                step=1,
                # initial_value=min(len(self.images), 20),
                initial_value=len(self.images),
    ...
    def visualize_frames(self):
        ...
        DEFAULT_COLOR = compute_rainbow_color()
    ...
    def load_data(self):
        ...
        self.splat_data = self.load_splat_file(self.splats_path, center=True)
    ...
    def load_splat_file(self, splat_path: Path, center: bool = False) -> SplatFile:
        ...
        bytes_per_gaussian = 3*4 + 3*4 + 4 + 4  # position + xyz + rgba + ijkl
    ...
    def initialize_visualizations(self):
        ...
        self.server.scene.add_transform_controls("/transform", visible=False)
    ...
    def reorient_scene(self):
        """Reorient the scene based on average camera up direction."""
        average_up = (
            vtf.SO3(np.array([img.qvec for img in self.images.values()]))
            @ np.array([0.0, -1.0, 0.0])  # -y is up in the local frame!
        ).mean(axis=0)
        average_up /= np.linalg.norm(average_up)
        self.server.scene.set_up_direction(tuple(average_up))
        self.status_text.text = "🟢 Scene reoriented based on camera poses"
    ...
    def add_point_cloud(self, points: np.ndarray, colors: np.ndarray):
    ...
    def add_frame(
        self, frame_id, height, width, fx, fy, qvec_wxyz, position, image, color
    ):
        frame = self.server.scene.add_frame(
            f"/frames/frame_{frame_id}",
            wxyz=qvec_wxyz,
            position=position,
            show_axes=False,
            axes_length=0.1,
        ...
        frustum = self.server.scene.add_camera_frustum(
            f"/frames/frame_{frame_id}/frustum",
...
def main(
    colmap_path: Path = Path(
        "/Users/chenyu/Desktop/datasets/bicycle/sparse/0"),
    images_path: Path = Path(
        "/Users/chenyu/Desktop/datasets/bicycle/images_8"),
    splats_path: Path = Path(
        "/Users/chenyu/Desktop/datasets/bicycle/bicycle.splat"),
    downsample_factor: int = 2,
    reorient_scene: bool = True,
) -> None:
