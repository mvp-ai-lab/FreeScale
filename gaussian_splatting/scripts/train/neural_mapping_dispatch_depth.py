...
PROJECT_ROOT_PATH = "freescale/gaussian_splatting" # Project path
DATASET_NAME = "DL3DV10K"
CONFIG_FILENAME = "cuscustom_depthtom"
...
def generate_3dgs_training_cmd(
    ...
    depth_log_path = os.path.join(scene_exp_out_dir, "depth_log.txt")
    ...
    model_folder = "manhattan_world" if use_manhattan_world else "0"
    depth_params_path = os.path.join(scene_path, f'sparse/{model_folder}/depth_params.json')

    depth_pred_cmd = f"cd {PROJECT_ROOT_PATH}/scripts/train && " + \
                     f"bash preprocess_depth.sh {gpu_idx} {scene_path} "
    if downsample > 1:
        depth_pred_cmd += f"{downsample} "
    depth_pred_cmd += f"> {depth_log_path} 2>&1 && "
    # Do not generate depth predictions if already did that.
    ...
    eval_cmd = f"python eval.py --config config/{CONFIG_FILENAME}.yaml " + \
                ...
                f"--init_ply_type sparse --scene {scene} " + \
                f"> {eval_log_path} 2>&1 && "
    cmd = depth_pred_cmd + train_cmd + eval_cmd
    ...
    if active_training:
        ...
        cmd = cmd + sample_cmd + active_training_cmd + active_eval_cmd + resample_cmd
