...
class Images2LatentScene(nn.Module):
    ...
    def forward(self, data_batch, has_target_image=True, ssl_train=False):
        ...
        if has_target_image and not ssl_train:
            loss_metrics = self.loss_computer(
                rendered_images,
                target.image,
                is_novel=target.is_novel
            )
        elif ssl_train:
            loss_metrics = None
            # FIXME!
    ...
    def forward_ssl(self, input, target):
    ...
    @torch.no_grad()
    def render_video(self, data_batch, data_path=None, traj_type="interpolate", num_frames=60, loop_video=False, order_poses=False):
        """
        ...
        if traj_type == "interpolate":
            ...
            ref_c2w = target.c2w[:, 0, ...]
            ref_fxfycxcy = target.fxfycxcy[:, 0, ...]
            device = target.c2w.device

            for b in range(bs):
                cur_c2ws, cur_intrinsics = camera_utils.generate_spiral_path(
                    ref_c2w[b], ref_fxfycxcy[b], num_frames
                )
                all_c2ws.append(cur_c2ws.to(device))        # [num_frames,3,4]
                all_intrinsics.append(cur_intrinsics.to(device))  # [num_frames,3,3]

        elif traj_type == "fixed":
            device = target.c2w.device
            # FIXME: set it to config afterwards
            rho_max = 0.174
            # 0.174=10deg, 0.349=20deg
            for b in range(bs):
                cur_c2ws, cur_intrinsics = camera_utils.generate_kubric_path(
                    target.fxfycxcy[:, 0, ...][b], target.scene_name[b], \
                    num_frames, target.avg_pose[b], target.scene_scale[b], \
                    rho_max=rho_max, z_motion=False, dataset_dir= data_path
                )
    ...
    def load_ckpt(self, load_path):
        ...
        try:
            ...
            print(f"--- [INFO] Loading checkpoint from {ckpt_paths[-1]}")
        except:
            traceback.print_exc()
