...
class PerceptualLoss(nn.Module):
    ...
    def _load_weights(self):
        ...
        weight_file = Path("FVGen/lvsm/metric_checkpoint/imagenet-vgg-verydeep-19.mat")
    ...
    @staticmethod
    def _compute_error_batch(real, fake):
        return torch.mean(torch.abs(real - fake), dim=[1, 2, 3])
    
    def forward(self, pred_img, target_img, novel_weights=None):
        ...
        if novel_weights is None:
            ...
            e0 = self._compute_error_batch(target_img_p, pred_img_p)
            e1 = self._compute_error_batch(target_features[0], pred_features[0]) / 2.6
            e2 = self._compute_error_batch(target_features[1], pred_features[1]) / 4.8
            e3 = self._compute_error_batch(target_features[2], pred_features[2]) / 3.7
            e4 = self._compute_error_batch(target_features[3], pred_features[3]) / 5.6
            e5 = self._compute_error_batch(target_features[4], pred_features[4]) * 10 / 1.5
        ...
        if novel_weights is None:
            ...
            total_loss = novel_weights * total_loss
...
class LossComputer(nn.Module):
    ...
    def forward(
        ...
        is_novel = is_novel.reshape(b * v)
        novel_weights = 1.0 - 0.7 * is_novel
        novel_weights = novel_weights.view(b * v, 1, 1, 1)
        ...
        if self.config.training.l2_loss_weight > 0.0:
            l2_loss = F.mse_loss(rendering, target, reduction='none')
            l2_loss = novel_weights * l2_loss
        ...
        if self.config.training.perceptual_loss_weight > 0.0:
            perceptual_loss = self.perceptual_loss_module(rendering, target, novel_weights)
    ...
    def add(
        ...
        """
        Add another loss metrics dictionary to this one.
        """
        added_loss_metrics = edict()
        for key in loss_metric_1.keys():
            if key == "psnr":
                added_loss_metrics[f"s1-{key}"] = loss_metric_1[key]
                added_loss_metrics[f"s2-{key}"] = loss_metric_2[key]
                continue
            # FIXME: maybe add 0.1 weight for loss 2????
            added_loss_metrics[key] = (loss_metric_1[key] + 0.1*loss_metric_2[key]) / 1.1

        return added_loss_metrics