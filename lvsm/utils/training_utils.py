...
def print_time(time_num):
    """Default time format is seconds"""
    if time_num < 60:
        return f"{time_num:.2f}s"
    elif time_num < 3600:
        return f"{time_num / 60:.2f}m"
    elif time_num < 86400:
        return f"{time_num / 3600:.2f}h"
    else:
        return f"{time_num / 86400:.2f}d"
...
def load_ckpt_weights_only(
    ...
    print(f"Loaded model weights from {load_path}, the status is {status}, rank: {torch.distributed.get_rank()}")
...
def auto_resume_job(
    ...
    if isinstance(model, DDP):
        ...
        status = model.load_state_dict(checkpoint['model'], strict=False)
    print_rank0(f"Loaded model weight from {os.path.abspath(ckpt_path)}, the status is {status}, rank: {torch.distributed.get_rank()}")
    ...
    return optimizer, lr_scheduler, scaler, forward_pass_step, param_update_step
